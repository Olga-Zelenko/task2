...
class Test2Iframe:
    def test_2_iframe(self):
        ...
        assert (
            nested_frames_page.is_open_page()
        ), "Страница с формой Nested frame не открылась"

        nested_frames_page.switch_iframe1_parent()
        assert (
            TestData.TEXT_IFRAME1_PARENT
            in nested_frames_page.getting_text_iframe1_parent()
        ), f"В центре страницы нет надписи {TestData.TEXT_IFRAME1_PARENT}"

        nested_frames_page.switch_iframe_child()
        assert (
            TestData.TEXT_IFRAME_CHILD in nested_frames_page.getting_text_iframe_child()
