...
class Test4Handles:
    def test_4_handles(self):
        ...
        assert browser_windows_page.is_open_page(), \
        ...
        assert len(list_window_handles_after) - len(list_window_handles_before) == 1, "Новая вкладка не появилась"
        assert TestData.URL_SAMPLE == browser_windows_page.get_relative_link(), f"Новая вкладка не имеет в url " \
                                                                                f"{TestData.URL_SAMPLE}"
